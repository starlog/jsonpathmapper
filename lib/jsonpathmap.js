...
exports.jsonpathmap = function (targetStructure, dataObject) {
  let obj = _.cloneDeep(targetStructure);
        ...
        switch (typeof obj[k]) {
          case "string":
            if (obj[k].startsWith("$")) {
              ...
              obj[k] = jp
                .query(document, "$" + obj[k].substring(1))
                .length.toString();
            } else if (obj[k].startsWith("%")) {
...
function handleArray(obj, document) {
  ...
  for (let k in obj) {
    if (obj.hasOwnProperty(k)) {
      ...
      if (
        testString !== null &&
        (testString.startsWith("$") || testString.startsWith("#"))
      ) {
        // Get total count
        ...
        for (let i = 0; i < count; i++) {
          ...
          for (let c in newElement) {
            if (newElement.hasOwnProperty(c)) {
              if (newElement[c].indexOf("[*]") !== -1) {
                newElement[c] = newElement[c].replace("*", i.toString());
              } // Assume range case
              else {
                let dataString = newElement[c].replace(/\s/g, "");
                let replaceString = dataString.substring(
                  dataString.indexOf("[") + 1,
                  dataString.indexOf("]")
                );
                let replaceArray = replaceString.split(",");
                newElement[c] = newElement[c].replace(
                  replaceString,
                  replaceArray[i].toString()
                );
              }
            }
          }
