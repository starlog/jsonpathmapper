                ...
                for (let i = 0; i < count; i++)
                {
                    ...
                    for (let c in newElement)
                    {
                        if (newElement.hasOwnProperty(c))
                        {
                            ...
                            else // Assume range case
                            {
                                let dataString = newElement[c].replace(/\s/g, '');
                                let replaceString = dataString.substring(dataString.indexOf('[')+1,dataString.indexOf(']'));
                                let replaceArray = replaceString.split(',');
                                newElement[c] = newElement[c].replace(replaceString, replaceArray[i].toString());
                            }
                        }
                    }
